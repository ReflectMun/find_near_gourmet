...
class _MainMapScreenState extends State<MainMapScreen> {
  ...
  // 地図のカメラが移されると
  void _editMarkerWhenCameraMoved(
  ...
  Future<Set<Marker>> _editMarker(
      {required double latitude, required double longitude}) async {
    // レストラン情報を取得
    final restaurantList = await GourmetApiService.getRestaurantListByLocation(
      lati: latitude,
      lngi: longitude,
      range: Provider.of<SearchConditionNotifier>(context, listen: false)
          .rangeDistance,
      page: 0,
      count: 100,
      genre: Provider.of<SearchConditionNotifier>(context, listen: false).genre,
      budget:
          Provider.of<SearchConditionNotifier>(context, listen: false).budget,
    );
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      body: SlidingUpPanel(
        ...
        panel: SearchConditionSettingPanel(
